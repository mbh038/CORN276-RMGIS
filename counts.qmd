...
* The data are in the form of raw counts for two or more categorical groups eg pea plants with either yellow peas or green peas, survival rate of mice if they took drug A or took drug B, people who expressed a preference for this or that type of landscape etc.
* Each independent observation must definitely belong to either one group or the other.
* There should be no replicates. That is, for each category we should have just have one count.
* The count values should all be greater than about 5. Any fewer than that and the conclusios of the analysis become unreliable.
...
#| caption: "Two plots of a chi-square distribution. The p-value for a given chi-square statistic is the shaded area under the curves, to the right of the statistic." 
#| layout-ncol: 2
...
```
We find that the chi-sqaure value is `r chi_sq`, which is very large. Remember that, the larger the chi-square value, the the further our observed values must be from the expected values, and thus, intuitibely, the more likely it must be that we will reject the null hypothesis.
...
```{r}
# first we get R to calculate the Chi-square statistic
chi_squared_statistic <- chisq.test(c(AA_o, Aa_o, aa_o), p = c(AA_e, Aa_e, aa_e)/N)$statistic
chi_squared_statistic
```

```{r}
# then we ask it to use that and the degrees of freedom to find the p-value
pchisq(246,df = 1, lower.tail = FALSE)
```

This is R's way of saying that the p-value is more or less zero. 

**Conclusion**
The p-value is very small indeed, much less than 0.05. Hence we reject the null hypothesis that the population is in Hardy-Weinberg equlibrium.
...
### Example - are two species independent?
...
1. It is valid here to use a chi square test for evidence of an association between species A and species B. Why?
...
2. What conclusion do we reach about the independence or otherwise of A and B?
