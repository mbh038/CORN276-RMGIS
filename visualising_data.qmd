...
Here is a box and whisker plot of the femal penguins body mass data, with one box for each species. Note that we have used the same colour for each box. We already know which species is which, so to use different colours would imply an additional difference that isn't there. It would confuse the reader.
...
#| fig-cap: "Box plots "
#| fig-subcap: 
#|   - "Anatomy of a box and whisker plot. Here the data are symmetrically distributed"
#|   - "Count date - not at all symmetrically distributed"
#| layout-ncol: 2
...
* whether there is a clear difference beteen the spread of values of the different catagories. This helps us decide whether or not to reject a null hypothesis about there being no difference. 
