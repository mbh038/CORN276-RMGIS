...
Quantile-Quantile plots, often known as Q-Q plots, can play a useful role when trying to decide whether a dataset is normally distributed, and if it is not, then how it differs from normality.
...
Below we show an example of 150 observations that are randomly drawn from a normal distribution. The normal distribution is symmetric, so it has no skew in either direction and its mean is equal to its median.

On a Q-Q plot, data that are approximately normally distributed data lie roughly on a straight line, perhaps looking a bit ragged at each end. The box plot is symmetric with few or no outliers.
