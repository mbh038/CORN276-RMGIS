...
A class of analytical models that are often used go under the name **General Linear Models**. They include linear regression, multiple regression, ANOVA, ANCOVA, Pearson correlation and t-tests.
...
Here, we will go through an example of simple linear regression - suitable for trend data where we wish to predict a continuously varying response, given a value of a single continuous explanatory variable.
...
Suppose that we have a data set to explore our question:
...
Data taken from [@balmer2011]

```{r}
pop <- read_csv(here("data","male_dolphins.csv"), skip = 2)
glimpse(pop)
```

```{r}
pop |>
  ggplot(aes(x=distance_km, y = POP_concentration)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Distance from chemicals plant (km)",
       y = "POP concentration in blubber") +
  scale_x_continuous(limits=c(0,90)) +
  theme_cowplot()
```

```{r}
pop.model <-lm(POP_concentration ~ distance_km, data = pop)
summary(pop.model)
```
* What is the change in POP concentration with each 1 km change in mean distance?
